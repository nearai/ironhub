...
- `records` (required): update_records: array of {id, fields}, e.g. [{"id":"recX","fields":{"Status":"Fixed"}}] (max 10).
...
The operation is selected by IronClaw as `airtable.update_records`. Do not send the private `action` selector.
