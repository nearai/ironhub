# DefiLlama: dex_overview

Run the legacy dex overview operation. See the prompt and input schema for its exact contract.
...
- `limit` (optional): Max DEXs, sorted by 24h volume desc.

The operation is selected by IronClaw as `defillama.dex_overview`. Do not send the private `action` selector.
