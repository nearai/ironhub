...
export default function SettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "Could not save the workspace name", "error")
    }
  }
    ...
    <div className="space-y-6">
      ...
      {isError && (
        <div className="flex items-start gap-2.5 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
          ...
          <span>
            Could not load this workspace{error instanceof Error ? `: ${error.message}` : "."}
          </span>
        </div>
      )}
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                disabled={!isOwner}
                className="max-w-md h-11 rounded-lg sm:h-10"
              />
              <p className="text-sm text-muted-foreground">
                This is the name shown in the workspace switcher and to everyone you invite.
              </p>
              {!isOwner && (
                <p className="text-sm text-muted-foreground">
            ...
            {isOwner && (
              <div className="border-t border-[var(--ironhub-line)] pt-4 flex flex-col-reverse sm:flex-row sm:justify-end">
                <Button
                  type="submit"
                  disabled={renameOrg.isPending || !name.trim()}
                  className="h-11 rounded-lg px-6 sm:h-10 w-full sm:w-auto"
                >
                  {renameOrg.isPending && <IconLoader2 className="size-4 animate-spin mr-1.5" />}
                  Save name
                </Button>
              </div>
            )}
