# NEAR Catalog: trending

Run the legacy trending operation. See the prompt and input schema for its exact contract.
...
The operation is selected by IronClaw as `nearcatalog.trending`. Do not send the private `action` selector.
