...
Run the legacy protocol tvl operation. See the prompt and input schema for its exact contract.
...
- `protocol` (required): Protocol slug, e.g. 'aave'. Returns the current TVL as a single number.

The operation is selected by IronClaw as `defillama.protocol_tvl`. Do not send the private `action` selector.
