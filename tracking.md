...
| `nova-submit` | live | 0.2.0 | Hackathon submission; encrypted file upload to a NOVA group | encryption, near, ipfs, hackathon, privacy, credentials-required | Encrypted submission to a NOVA group on NEAR. Self-joins an open submission group, then performs session-token, prepare_upload, client-side AES-256-GCM encryption, and finalize_upload in one call. Submissions are manifests (title, description, repo/video links), 1 MB cap. | Requires a NOVA account + API key (nova-sdk.com). The submission group must be opened for join by its owner (`open_hackathon_join`); if not open, the tool returns "Group not open for join" — no manual member-add is needed. Deterministic per-file encryption nonce derived from the upload_id. | jcarbonnell |
