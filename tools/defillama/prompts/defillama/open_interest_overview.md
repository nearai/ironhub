# DefiLlama: open_interest_overview

Run the legacy open interest overview operation. See the prompt and input schema for its exact contract.
...
- `limit` (optional): Max perp protocols, sorted by open interest desc.

The operation is selected by IronClaw as `defillama.open_interest_overview`. Do not send the private `action` selector.
