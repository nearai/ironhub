### 1. Title

Media Downloader media using yt-dlp

### 2. Example prompt

Action + URL

Download this video https://x.com/karanvaidya6/status/2041516353551737338?s=46
<img width="679" height="289" alt="Image" src="https://github.com/user-attachments/assets/05ba7706-60fd-4492-ae6b-7eee2589e560" />

Download this https://www.youtube.com/watch?v=F-zCxQPLQ8Q as mp3
<img width="675" height="631" alt="Image" src="https://github.com/user-attachments/assets/3e9dd0ed-f3cd-4d6b-b4d6-58f02f9e29cb" />

### 3. What the agent does

User gets a media file sent to their chat interface. Once the video has been downloaded and sent to the user; the agent should delete the file from its system to avoid maxing out on the server storage.

### 4. Skills & tools used

- References this Github repo: https://github.com/yt-dlp/yt-dlp — Required by this use case.

### 5. Categories

- [ ] Personal assistant
- [ ] Web 3 / Crypto
- [ ] Coding / dev workflow
- [ ] Research
- [x] Marketing / content
- [ ] Business ops
- [ ] Sales / CRM
- [x] Files / knowledge
- [ ] Automation
- [x] Design / media
- [ ] Skill creation

### 6. Source (optional)

_No response_

### 7. Author (optional)

@fiatisabubble
