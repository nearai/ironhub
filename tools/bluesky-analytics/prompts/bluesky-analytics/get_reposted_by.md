# Bluesky Analytics: get_reposted_by

Run the legacy get reposted by operation. See the prompt and input schema for its exact contract.
...
The operation is selected by IronClaw as `bluesky-analytics.get_reposted_by`. Do not send the private `action` selector.
