...
Run the legacy tokentx operation. See the prompt and input schema for its exact contract.
...
- `contractaddress` (optional): ERC-20 token contract address. Optional.
...
The operation is selected by IronClaw as `etherscan.tokentx`. Do not send the private `action` selector.
