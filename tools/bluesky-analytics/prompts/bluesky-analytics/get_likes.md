# Bluesky Analytics: get_likes

Run the legacy get likes operation. See the prompt and input schema for its exact contract.
...
The operation is selected by IronClaw as `bluesky-analytics.get_likes`. Do not send the private `action` selector.
