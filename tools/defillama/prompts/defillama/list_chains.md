# DefiLlama: list_chains

Run the legacy list chains operation. See the prompt and input schema for its exact contract.
...
- `limit` (optional): Max chains, sorted by TVL desc.

The operation is selected by IronClaw as `defillama.list_chains`. Do not send the private `action` selector.
