...
#[cfg(any(not(feature = "reborn"), test))]
...
/// Adapt Reborn's per-capability public surface to the existing internal
/// tagged command enum. The raw passthrough command intentionally has no
/// capability mapping: typed operations keep auth and approval boundaries
/// reviewable.
#[cfg(feature = "reborn")]
fn execute_reborn(params: &str, context: Option<&str>) -> Result<String, String> {
    ...
    let command = match context.capability_id.as_str() {
        "wordpress.list_posts" => "list_posts",
        "wordpress.get_post" => "get_post",
        "wordpress.create_post" => "create_post",
        "wordpress.update_post" => "update_post",
        "wordpress.delete_post" => "delete_post",
        "wordpress.upload_media" => "upload_media",
        "wordpress.list_media" => "list_media",
        "wordpress.get_media" => "get_media",
        "wordpress.update_media" => "update_media",
        "wordpress.delete_media" => "delete_media",
        "wordpress.list_products" => "list_products",
        "wordpress.get_product" => "get_product",
        "wordpress.create_product" => "create_product",
        "wordpress.update_product" => "update_product",
        "wordpress.delete_product" => "delete_product",
        "wordpress.list_orders" => "list_orders",
        "wordpress.get_order" => "get_order",
        "wordpress.update_order" => "update_order",
        "wordpress.list_customers" => "list_customers",
        _ => return Err("unsupported_wordpress_capability".to_string()),
    };
    ...
    // Reborn's package is pinned to the default REST prefix. A caller must not
    // reintroduce the legacy raw selector or an arbitrary route namespace.
    if object.contains_key("command") || object.contains_key("api_prefix") {
...
mod tests {
    ...
    #[cfg(feature = "reborn")]
    #[test]
    fn reborn_context_selects_typed_command_and_rejects_legacy_controls() {
        let error = execute_reborn(
            r#"{"command":"delete_post","site_url":"shop.example.com","id":7}"#,
            Some(r#"{"capability_id":"wordpress.get_post"}"#),
        )
        .unwrap_err();
        assert_eq!(error, "invalid_parameters");

        let error = execute_reborn(
            r#"{"site_url":"shop.example.com","api_prefix":"/other/"}"#,
            Some(r#"{"capability_id":"wordpress.list_posts"}"#),
        )
        .unwrap_err();
        assert_eq!(error, "invalid_parameters");

        let error = execute_reborn(
            r#"{"site_url":"shop.example.com"}"#,
            Some(r#"{"capability_id":"wordpress.wp_request"}"#),
        )
        .unwrap_err();
        assert_eq!(error, "unsupported_wordpress_capability");
    }
}
