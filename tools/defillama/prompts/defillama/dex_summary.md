# DefiLlama: dex_summary

Run the legacy dex summary operation. See the prompt and input schema for its exact contract.
...
- `protocol` (required): DEX protocol slug, e.g. 'uniswap'.

The operation is selected by IronClaw as `defillama.dex_summary`. Do not send the private `action` selector.
