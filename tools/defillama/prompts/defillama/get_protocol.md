...
Run the legacy get protocol operation. See the prompt and input schema for its exact contract.
...
- `protocol` (required): Protocol slug, e.g. 'aave', 'uniswap'. Discover slugs with list_protocols.
- `points` (optional): Max TVL history points returned (downsampled).

The operation is selected by IronClaw as `defillama.get_protocol`. Do not send the private `action` selector.
