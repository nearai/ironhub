...
Run the legacy getabi operation. See the prompt and input schema for its exact contract.
...
The operation is selected by IronClaw as `etherscan.getabi`. Do not send the private `action` selector.
