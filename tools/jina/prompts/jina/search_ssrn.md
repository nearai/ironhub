...
Search SSRN papers with Jina Search.
...
- `query` (required): Search query for academic papers on SSRN (Social Science Research Network).
- `num` (optional): The number of search results to return (default 30).

The operation is selected by IronClaw as `jina.search_ssrn`. Do not send the private `action` selector.
