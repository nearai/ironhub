# Etherscan: gettxreceiptstatus

Run the legacy gettxreceiptstatus operation. See the prompt and input schema for its exact contract.
...
The operation is selected by IronClaw as `etherscan.gettxreceiptstatus`. Do not send the private `action` selector.
