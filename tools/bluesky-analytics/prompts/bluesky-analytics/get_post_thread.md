# Bluesky Analytics: get_post_thread

Run the legacy get post thread operation. See the prompt and input schema for its exact contract.
...
- `uri` (required): Post at-uri (e.g. 'at://did:plc:.../app.bsky.feed.post/<rkey>'). Obtain one from get_author_feed output.
- `depth` (optional): Reply-tree depth (default 6, max 100).

The operation is selected by IronClaw as `bluesky-analytics.get_post_thread`. Do not send the private `action` selector.
