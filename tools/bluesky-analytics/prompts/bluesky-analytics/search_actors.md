# Bluesky Analytics: search_actors

Run the legacy search actors operation. See the prompt and input schema for its exact contract.
...
- `q` (required): Keyword query (matches handle/display name/bio).
- `limit` (optional): Max results (1-100, default 50).
- `cursor` (optional): Opaque pagination cursor from a previous response. Pass it back to fetch the next page.

The operation is selected by IronClaw as `bluesky-analytics.search_actors`. Do not send the private `action` selector.
