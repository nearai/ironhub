# DefiLlama: get_stablecoin

Run the legacy get stablecoin operation. See the prompt and input schema for its exact contract.
...
- `stablecoin_id` (required): Numeric stablecoin id (e.g. '1' for USDT). Discover ids with list_stablecoins.

The operation is selected by IronClaw as `defillama.get_stablecoin`. Do not send the private `action` selector.
