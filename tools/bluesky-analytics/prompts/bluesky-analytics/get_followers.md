# Bluesky Analytics: get_followers

Run the legacy get followers operation. See the prompt and input schema for its exact contract.
...
The operation is selected by IronClaw as `bluesky-analytics.get_followers`. Do not send the private `action` selector.
