...
Run the legacy shopping operation. See the prompt and input schema for its exact contract.

Legacy tool context: Google Search, News, Images, Videos, Places, and Shopping API tool via Serper.dev

## Inputs

- `q` (required): Google Shopping query.
...
The operation is selected by IronClaw as `serper.shopping`. Do not send the private `action` selector.
