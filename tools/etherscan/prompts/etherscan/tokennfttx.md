# Etherscan: tokennfttx

Run the legacy tokennfttx operation. See the prompt and input schema for its exact contract.
...
- `contractaddress` (optional): ERC-721 token contract address. Optional.
...
The operation is selected by IronClaw as `etherscan.tokennfttx`. Do not send the private `action` selector.
