# DefiLlama: historical_prices

Run the legacy historical prices operation. See the prompt and input schema for its exact contract.
...
- `timestamp` (required): Unix timestamp (seconds) of the moment to price.
- `search_width` (optional): Time range to find price data, e.g. '4h' (default 6h).

The operation is selected by IronClaw as `defillama.historical_prices`. Do not send the private `action` selector.
