# DefiLlama: first_prices

Run the legacy first prices operation. See the prompt and input schema for its exact contract.
...
- `coins` (required): Comma-separated '{chain}:{address}' or 'coingecko:{id}'. Returns each token's earliest recorded price.

The operation is selected by IronClaw as `defillama.first_prices`. Do not send the private `action` selector.
